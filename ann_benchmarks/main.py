...
def run_worker(
    cpu: int, args: argparse.Namespace, queue: multiprocessing.Queue
) -> None:
    """
    ...
    while not queue.empty():
        ...
        if args.local:
            ...
            mem_limit = int(
                (psutil.virtual_memory().available - memory_margin)
                / args.parallelism
            )
            cpu_limit = (
                str(cpu)
                if not args.batch
                else f'0-{multiprocessing.cpu_count() - 1}'
            )
    ...
    for definition in definitions:
        not_yet_run = [
            ...
            or not os.path.exists(
                build_result_filepath(
                    dataset, count, definition, query_args, batch
                )
            )
        ]
...
def create_workers_and_execute(
    definitions: List[Definition], args: argparse.Namespace
):
    """
    ...
    try:
        workers = [
            multiprocessing.Process(
                target=run_worker, args=(i + 1, args, task_queue)
            )
...
def filter_disabled_algorithms(
    definitions: List[Definition],
) -> List[Definition]:
    """
...
def limit_algorithms(
    definitions: List[Definition], limit: int
) -> List[Definition]:
    """
...
def main():
    ...
    if not args.local:
        ...
        definitions = list(
            filter(check_module_import_and_constructor, definitions)
        )
